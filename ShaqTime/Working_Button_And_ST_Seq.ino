...
rgb_color readyColor;

bool shaqReadyInit = 1;
...
void loop()
{
		...
		if (keyOn() == 1) {
				...
				// Run Shaq Ready
				if (shaqTime == 0) {					
						// Generate random colors in the center LEDs
						unsigned int rMax = random(255);
						unsigned int gMax = random(1);
						unsigned int bMax = random(1);
						//colors[LED_COUNT / 2 - 1] = rgb_color{ random(rMax), random(gMax), random(bMax) };
						//colors[LED_COUNT / 2] = rgb_color{ random(rMax), random(gMax), random(bMax) };
						//colors[LED_COUNT / 2 - 1] = rgb_color{ rMax, gMax, bMax };
						//colors[LED_COUNT / 2] = rgb_color{ rMax, gMax, bMax };
						int omega = 2;
						/*downstream (right)*/
						colors[LED_COUNT / 2] = rgb_color{ 120 + 120 * sin(theta_r), gMax, 120 + 120 * sin(theta_b + 0.2) };
						/*closest to arduino (left)*/
						colors[LED_COUNT / 2 - 1] = rgb_color{ 0 + 64 * sin(theta_r), gMax, 0 + 64 * sin(theta_b) };

						//colors[LED_COUNT / 2 - 1] = rgb_color{ rMax, gMax, bMax };
						//gummy worm
						//colors[LED_COUNT / 2] = rgb_color{128 + 128 * sin(theta_r), 128 + 128 * sin(theta_g),  bMax};
						//robocop
						//colors[LED_COUNT / 2] = rgb_color{128 + 128 * sin(theta_r), gMax, 128 + 128 * sin(theta_b)};
						...
						// Propagate colors from the center outwards
...
void shaqShimmer(unsigned char dimOnly)
{
		...
		for (int i = 0; i < LED_COUNT; i += 2)
		{
				...
				colors[i].green = colors[i].red >> 4;  // green = 80% of red
				colors[i].blue = colors[i].red >> 4;  // blue = red/8
...
robo-cop
colors[LED_COUNT / 2] = rgb_color{ 64 + 64 * sin(theta_r), gMax, 64 + 64 * sin(theta_b) };
		pink-blue variant
		colors[LED_COUNT / 2] = rgb_color{ 0 + 64 * sin(theta_r), gMax, 0 + 64 * cos(theta_b) };
		red-blue variant
		colors[LED_COUNT / 2] = rgb_color{ 0 + 64 * sin(theta_r), gMax, 0 + 64 * sin(theta_b) };
